...
std::ostream& operator<<(std::ostream& os, const Continent& c) {
	...
	os << "id: " << *c.id << ", name: " << *c.name << ", bonus valus: " << *c.bonusValue
		<< ", territories ids: [";
...
std::ostream& operator<<(std::ostream& os, const Territory& t) {
	...
	os << "id: " << *t.id << ", name: " << *t.name << ", continent id: " << *t.continentId
		<< ", player id: " << *t.playerId << ", armies number: " << *t.armiesNum
		<< ", neighbors ids: [";
...
MapLoader::MapLoader(string filename) {
	...
	while (getline(file, line)) {
		...
		// changing sections
		if (line[0] == '[') {
			if (line == "[continents]") section = Section::Continents;
		...
		if (section == Section::Invalid) continue;
	...
	while (getline(file, line)) {
		...
		if (section == Section::Continents) {
			...
			while (ss >> neighborId) {
				(*map->territories)[territoryId - 1]->neighborsIds->push_back(new int(neighborId - 1));
			}
		}
...
void Map::validate() {
	...
	for (int i = 0; i < continentsNum; i++) {
		...
		while (!s.empty()) {
			...
			visited[u] = true;
			for (auto v : *(*territories)[u]->neighborsIds) {
				if (*(*territories)[*v]->continentId == i && !visited[*v]) s.push(*v);
			}
		}
...
bool MapLoader::isValid() {
	return *isFormatValid && map != nullptr && *map->validateErr == Map::Err::None;
}
...
int MapLoader::getContinentsNum() {
	return (int) map->continents->size();
}

int MapLoader::getTerritoriesNum() {
	return (int) map->territories->size();
}
