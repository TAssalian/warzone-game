...
bool Order::getIsExecuted() const{
    return isExecuted;
}

std::string Order::getOrderEffect() const{
    return orderEffect;
}
...
std::ostream& operator<<(std::ostream& os, const Order& order) {
    os << "Order Type: " << order.getOrderType()
       << ", Issuer: " << (order.getPlayer() ? order.getPlayer()->getName() : "None")
...
//Parameterized Constructor
DeployOrder::DeployOrder(Player* issuer,int numArmies, Territory* targetTerritory):
    Order("Deploy",issuer),
    numArmies(numArmies),
...
bool DeployOrder::execute(){
    if(validate()){
        setIsExecuted(true);
        setEffect("Deployed " + to_string(numArmies) + " armies to " + *(targetTerritory->getName()));
        return true;
    }else{
        setIsExecuted(false);
...
//Parameterized Constructor
AdvanceOrder::AdvanceOrder(Player* issuer,int numArmies, Territory* source, Territory* target):
...
Territory* AdvanceOrder::getSourceTerritory(){
    return source;
}

Territory* AdvanceOrder::getTargetTerritory(){
    return target;
}
...
bool AdvanceOrder::execute(){
    if(validate()){
        setIsExecuted(true);
        setEffect("Advanced " + to_string(numArmies) + " armies between friendly territories from " + *(source->getName()) + " to " + *(target->getName()));
        return true;
    }else{
        setIsExecuted(false);
...
//Setters
void BombOrder::setTargetTerritory(Territory* target){
    this->target=target;
}
...
//Default Constructor
BlocakdeOrder::BlocakdeOrder():
    Order("Blockade", nullptr),
    target(nullptr)
{}  

//Parameterized Constructor
BlocakdeOrder::BlocakdeOrder(Player* issuer,Territory* target):
    Order("Blockade", issuer),
    target(target)
{}

//Getters
Territory* BlocakdeOrder::getTargetTerritory() const{
    return target;
}

//Setters
void BlocakdeOrder::setTargetTerritory(Territory* target){
    this->target=target;
}

//Methods
bool BlocakdeOrder::validate(){
    return true;
}

bool BlocakdeOrder::execute(){
    if(validate()){
        setIsExecuted(true);
        setEffect("Blockaded territory " + *(target->getName()) + ", tripling its armies and making it neutral.");
        return true;
    }else{
        setIsExecuted(false);
...
//Parameterized Constructor
AirliftOrder::AirliftOrder(Player* issuer,int numArmies, Territory* target, Territory* source):
...
Territory* AirliftOrder::getSourceTerritory(){
    return source;
}

Territory* AirliftOrder::getTargetTerritory(){
    return target;
}
...
bool AirliftOrder::execute(){
    if(validate()){
        setIsExecuted(true);
        setEffect("Airlifted " + to_string(numArmies) + " armies from " + *(source->getName()) + " to " + *(target->getName()));
        return true;
    }else{
        setIsExecuted(false);
...
//Setters
void NegotiateOrder::setTargetPlayer(Player* target){
    this->target=target;
} 
...
bool NegotiateOrder::execute(){
    if(validate()){
        ...
        setIsExecuted(false);
        setEffect("Negotiate order validation failed. Order has not been executed.");
        return false;
    }
}
...
void OrderList::move(int currentIndex, int newIndex){
    if (currentIndex < newIndex) {
        ...
        std::rotate(orders->begin() + newIndex,
                    orders->begin() + currentIndex,
                    orders->begin() + currentIndex + 1);
    }
}
