...
int run_game_engine_driver() {
	...
	cout << "  Execute Orders    -> exec-order(7) | end-exec-orders(8) | win(9)" << endl;
	...
	while (running) {
		...
		cout << "Command " << (gameEngine->validateCommand(command) ? "valid" : "invalid") << " for current state." << endl;
		cout << endl; cout << endl;
		if(!validtransition) {
		...
		if((command == "end" || "11") && gameEngine->getCurrentState() == 7) {
			running = false;
		}
