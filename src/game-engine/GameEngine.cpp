...
GameState stringToGameState(const std::string& str) {
	if (str == "load-map"			|| str == "1")  return MAP_LOADED;
	if (str == "validate-map"		|| str == "2")  return MAP_VALIDATED;
	if (str == "add-player"			|| str == "3")  return PLAYERS_ADDED;
	if (str == "assign-countries"	|| str == "4")  return ASSIGN_REINFORCEMENT;

	if (str == "issue-order"		|| str == "5")  return ISSUE_ORDERS;
	if (str == "end-issue-orders"	|| str == "6")  return EXECUTE_ORDERS;
	if (str == "exec-order"			|| str == "7")  return EXECUTE_ORDERS;
	if (str == "end-exec-orders"	|| str == "8")  return ASSIGN_REINFORCEMENT;

	if (str == "win"				|| str == "9")  return WIN;
	if (str == "play"				|| str == "10") return START;
	if (str == "end"				|| str == "11") return WIN; 
...
bool GameEngine::validateCommand(string& command) {
	...
	case EXECUTE_ORDERS:
		return command == "exec-order" || command == "7"
			|| command == "end-exec-orders" || command == "8"
			|| command == "win" || command == "9";
	case WIN:
		return command == "play" || command == "10"
			|| command == "end" || command == "11";
	default:
		return false;
	}
}
